...
def parse_reloc32(data: bytes, reloff: int, nrel: int, entsz: int, syms: Sequence[Sym], rela: bool) -> Reloc:
    ...
    for off in range(reloff, reloff+entsz*nrel, entsz):
        ...
        symind = inf >> 8
        sym = syms[symind] if symind < len(syms) else symind
        type = inf & 0xff
...
def parse_32(data: bytes) -> ELF:
    ...
    return ELF(data, ident, eclass, etype, mach, entry, phdrs, dyn, shdrs,
               symtab, dynsym, relocs, notes, True)


def parse_note64(data: bytes, nphdr: Phdr) -> Sequence[Phdr]:
    # format is same as note32, so eh
    return parse_note32(data, nphdr)
...
def parse_64(data: bytes) -> ELF:
    ...
    return ELF(data, ident, eclass, etype, mach, entry, phdrs, dyn, shdrs,
               symtab, dynsym, relocs, notes, False)
