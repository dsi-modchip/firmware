...
unsigned char *Load(const char *filename, unsigned int *length, int min, int max);
void  Save(const char *filename, unsigned char *buffer, int length);
unsigned char *Memory(int length, int size);

void  LZS_Decode(const char *filename, const char* outfilename);
void  LZS_Encode(const char *filename, const char* outfilename, int mode);
...
int main(int argc, char **argv) {
  ...
  switch (cmd) {
    case CMD_DECODE:
      LZS_Decode(argv[2], outfile);
      break;
    case CMD_CODE_10:
      LZS_Encode(argv[2], outfile, mode);
      break;
    default:
      break;
  }
...
void Usage(void) {
  EXIT(
    "Usage: LZSS command filename outfilename\n"
    "\n"
    "command:\n"
    "  -d ..... decode 'filename'\n"
...
void LZS_Decode(const char *filename, const char* outfilename) {
  ...
  Save(outfilename, raw_buffer, raw_len);
